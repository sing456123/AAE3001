...
Optimise: piecewise-linear N-point chord and twist distributions for max CT at 250 kts
"""
...
# Twist is now referenced to the design-point inflow angle instead of fixed
...
print(f"Reference polar conditions (75% span, 250 kts):")
print(f"  V_local = {V_loc_75:.1f} m/s  M = {M_ref:.3f}  Re = {Re_ref/1e6:.2f}M")
print("  Running NeuralFoil for NACA 9112...")

aero   = nf.get_aero_from_airfoil(airfoil=airfoil, alpha=ALPHA_POLAR,
                                   Re=Re_ref, model_size="large")
CL_raw = np.array(aero["CL"])
CD_raw = np.array(aero["CD"])

CL_func = interp1d(ALPHA_POLAR, CL_raw, bounds_error=False,
                   fill_value=(CL_raw[0], CL_raw[-1]))
CD_func = interp1d(ALPHA_POLAR, CD_raw, bounds_error=False,
                   fill_value=(CD_raw[0], CD_raw[-1]))

print(f"  CL_max = {CL_raw.max():.3f}  at α = {ALPHA_POLAR[CL_raw.argmax()]:.1f}°")
print(f"  Best L/D = {(CL_raw/CD_raw).max():.1f}  at α = {ALPHA_POLAR[(CL_raw/CD_raw).argmax()]:.1f}°")
phi_ctrl_deg = np.degrees(np.arctan2(lambda_c_design, (R_root / R) + GEOM_CTRL_STATIONS * (1.0 - R_root / R)))
print(
    f"  Design inflow angle range (no induced flow): "
    f"{phi_ctrl_deg[0]:.1f}° at root to {phi_ctrl_deg[-1]:.1f}° at tip"
)
...
def solve_inflow(r, sigma, theta_rad, lambda_c):
    def residual(lambda_i):
        ...
        # Standard actuator disk: 4 lambda_i lambda r = CT_blade
...
def bem_solver(design_vars, V_c, N=N_EL):
    """
    Full BEM with drag.
    Inflow ratio λ solved iteratively via brentq at each element.
...
# ============================================================
# 5. Optimisation — Max CT at 250 kts
# ============================================================
def objective(x):
    CT, *_ = bem_solver(x, V_DESIGN)
    return -CT

print(
    f"\nOptimising piecewise-linear propeller geometry with {N_CTRL} control points "
...
print(f"\n{'='*52}")
print("OPTIMAL PROPELLER GEOMETRY  (NACA 9112)")
print(f"{'='*52}")
...
for label, chord, twist in zip(GEOM_CTRL_LABELS, opt_chord_ctrl, opt_twist_ctrl):
    print(f"  {label:8}: chord = {chord*100:.1f} cm   twist = {twist:.1f}°")
...
for kts in [200, 250, 300]:
    ...
    print(f"{kts:>7} kts  {CT:>9.5f}  {CQ:>9.5f}  {eta:>7.3f}  {T:>12.1f}  {P:>11.1f}")
...
ax.set_xlabel("Radius [m]"); ax.set_ylabel("Chord [cm]")
ax.set_title("Chord Distribution"); ax.grid(True, alpha=0.3)

# Twist distribution
ax = axes[0, 1]
ax.plot(r_phys, th_arr, color="tomato", linewidth=2)
ax.scatter(ctrl_r_phys, opt_twist_ctrl, color="darkred", s=30, zorder=3)
ax.set_xlabel("Radius [m]"); ax.set_ylabel("Twist angle [deg]")
ax.set_title("Twist Distribution"); ax.grid(True, alpha=0.3)

# Inflow angle
ax = axes[1, 0]
ax.plot(r_phys, phi_dist, color="seagreen", linewidth=2, label="Inflow angle φ")
ax.plot(r_phys, th_arr, color="tomato", linewidth=1.5, linestyle="--", label="Blade pitch θ")
ax.set_xlabel("Radius [m]"); ax.set_ylabel("Angle [deg]")
ax.set_title("Inflow vs Pitch Angle"); ax.legend(); ax.grid(True, alpha=0.3)

# Effective AoA
ax = axes[1, 1]
ax.plot(r_phys, alpha_dist, color="purple", linewidth=2)
ax.axhline(ALPHA_POLAR[CL_raw.argmax()], color="tomato", linestyle="--",
           linewidth=1.5, label=f"Stall α = {ALPHA_POLAR[CL_raw.argmax()]:.1f}°")
ax.set_xlabel("Radius [m]"); ax.set_ylabel("Effective AoA [deg]")
ax.set_title("Effective Angle of Attack along Blade"); ax.legend(); ax.grid(True, alpha=0.3)

plt.suptitle(
    f"NACA 9112 Propeller — R={R}m, {N_b} blades, {RPM} RPM\n"
    f"{N_CTRL}-point piecewise-linear chord/twist control   "
    f"Root->Tip chord {opt_chord_ctrl[0]*100:.1f}->{opt_chord_ctrl[-1]*100:.1f} cm   "
    f"Twist {opt_twist_ctrl[0]:.1f}->{opt_twist_ctrl[-1]:.1f}°",
    fontsize=11, fontweight="bold"
)
plt.tight_layout()
plt.savefig("propeller_design.png", dpi=150, bbox_inches="tight")
print("\nPlot saved: propeller_design.png")
