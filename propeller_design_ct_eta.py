...
Optimise: piecewise-linear N-point chord and twist distributions for max CT
subject to an eta band at 250 kts
"""
...
# Efficiency band for constrained max-CT optimisation.
# Adjust these two values to tighten or relax the acceptable eta range.
ETA_MIN = 0.92
ETA_MAX = 0.95

INVALID_DESIGN_PENALTY = 1e6
...
print(f"  Eta band constraint: {ETA_MIN:.2f} <= eta <= {ETA_MAX:.2f}")
...
def safe_eta(x):
    _, _, eta = bem_solver(x, V_DESIGN)
    return eta if np.isfinite(eta) else -INVALID_DESIGN_PENALTY


# ============================================================
# 5. Optimisation — Max CT inside eta band
# ============================================================
def objective(x):
    CT, _, eta = bem_solver(x, V_DESIGN)
    if not np.isfinite(eta) or not np.isfinite(CT):
        return INVALID_DESIGN_PENALTY
    return -CT
...
constraints.extend(
    [
        {"type": "ineq", "fun": lambda x: safe_eta(x) - ETA_MIN},
        {"type": "ineq", "fun": lambda x: ETA_MAX - safe_eta(x)},
    ]
)

print(
    f"\nOptimising piecewise-linear propeller geometry for max CT with {N_CTRL} control points "
    f"per distribution (design point: 250 kts, eta-banded)..."
)
...
print(f"\n{'='*60}")
print("OPTIMAL PROPELLER GEOMETRY  (NACA 9112, MAX CT WITH ETA BAND)")
print(f"{'='*60}")
...
print(f"  Eta band    : {ETA_MIN:.2f} <= eta <= {ETA_MAX:.2f}")
...
plt.suptitle(
    ...
    f"{N_CTRL}-point piecewise-linear chord/twist control (max CT, eta-banded)   "
...
plt.tight_layout()
plt.savefig("propeller_design_ct_eta.png", dpi=150, bbox_inches="tight")
print("\nPlot saved: propeller_design_ct_eta.png")
